...
//req is input, res is output
async function bestWinePerPrice(req, res) {
  console.log("entered");
  pricename = parseInt(req.params.price);
  ...
  try {
    ...
    let result = await connection.execute(
        // The statement to execute
      ` WITH winery_prevalence AS (
        SELECT DISTINCT winery, province
        from wine_origin),
        winery_info AS(
        select winery, count (Province) AS winery_spread
        FROM winery_prevalence
        GROUP BY winery),
        avg_winery_num_info AS (
        select avg(winery_spread) AS avg_winery_num
        from winery_info),
        winery_final_count AS (
        SELECT wi.winery
        from winery_info wi
        WHERE wi.winery_spread >= (select avg_winery_num from avg_winery_num_info)),
        rating AS (
        select title, avg(points) AS avg_rating, count(id) As num_reviews
        from wine_review
        group by title
        order by avg_rating), num_reviews_info AS (
        select avg(num_reviews) AS avg_num_reviews
        FROM rating)
        SELECT  w.title AS wine,w.winery,r.avg_rating
        FROM wine_origin w
        JOIN rating r
        ON w.title = r.title
        WHERE
        w.price between (:bnbv) and (:bnbv + 9)
        AND w.winery IN (SELECT winery FROM winery_final_count)
        AND r.num_reviews >= (select avg_num_reviews from num_reviews_info)
...
module.exports = {
  ...
  getPrices: getPrices,
  bestWinePerPrice: bestWinePerPrice
}
